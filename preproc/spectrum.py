...
class Spectrum(object):
    ...
    def __init__(self, spec: 'Spectrum' = None, copy_flux: bool = True, wave: np.ndarray = None, teff: float = None,
                 logg: float = None, feh: float = None, param: np.ndarray = None,
    ...
    @property
    def teff(self) -> float:
        """Returns the Effective temperature in K for this spectrum
        Returns:
             Teff in Kelvin.
        """
        return self._teff
    ...
    @property
    def logg(self) -> float:
        """Returns the log of surface gravity in cm/s*s for this spectrum
        Returns:
             logg in dex.
        """
        return self._logg
    ...
    @property
    def feh(self) -> float:
        """Returns the metallicity for this spectrum
        Returns:
             [Fe/H] in dex.
        """
        return self._feh
    ...
    def param(self) -> np.ndarray:
        """Returns an array of all atmospheric parameters associated with the current spectrum
        Returns:
    ...
    def resample(self, spec: 'Spectrum' = None, 
                 wave: np.ndarray = None, wave_start: float = None, wave_count: int = None, wave_step: float = None, wave_end: float = None,
                 vrad: float = 0., linear: bool = False, flux_conserve: bool = False, fill_value: float = np.nan) -> 'Spectrum':
    ...
    def _resample(spec: 'Spectrum', wave: np.ndarray, flux_conserve: bool = False, linear: bool = False,
                  fill_value: float = np.nan) -> np.ndarray:#Union[Union[List[None], np.ndarray, Iterable, int, float], Any]:
    ...
    def smooth(self, fwhm: float = None, sigma: float = None):
        ...
        # smooth
        self.flux = convolve(self.flux, gauss)
    ...
    def index_of_wave(self, wave: float) -> int:
        ...
        # too small/large?
        if wave < w[0]:
            raise ValueError("Starting wavelength point is out of range")
        if wave > w[-1]:
            raise ValueError("End wavelength point is out of range")
    ...
    def __init__(self, hdu=None, hdu_list=None, primary=True, dtype=np.float32, *args, **kwargs):
        ...
        if 'spec' in kwargs and isinstance(kwargs['spec'], SpectrumFitsHDU):
            ...
            if hdu:
                ...
                # get header
                hdr = hdu.header
                self._teff = hdr["TEFF"] if "TEFF" in hdr else hdr["PHXTEFF"]
                self._logg = hdr["LOGG"] if "LOGG" in hdr else hdr["PHXLOGG"]
                self._feh = hdr["FEH"] if "FEH" in hdr else hdr["PHXM_H"]
                self._param = np.array([self._teff, self._logg, self._feh])
